...
1. [Releases](../../releases) 페이지에서 **`Kidsnote_Memories_Saver_V1.00.exe`** 를 다운로드합니다
2. 다운로드한 `.exe` 파일을 더블클릭하여 실행합니다
3. **Chrome 브라우저가 컴퓨터에 설치되어 있어야 합니다** (별도의 ChromeDriver는 필요 없습니다!)

> [!WARNING]
> ### 🛡️ Windows SmartScreen / Edge 차단 시 해결 방법
>
> 개인이 만든 무료 프로그램은 코드 서명 인증서가 없어 SmartScreen이 차단할 수 있습니다.  
> 소스 코드가 이 저장소에 **완전히 공개**되어 있으므로 안심하고 사용하셔도 됩니다.
>
> #### Edge 다운로드 차단 시
> 1. Edge 오른쪽 상단 **`...`** → **다운로드** 클릭
> 2. 파일 옆 **`...`** → **보관(Keep)** 클릭
> 3. "여전히 위험할 수 있습니다" 경고 → **추가 정보 보기** → **그래도 유지** 클릭
>
> #### 실행 시 "Windows가 PC를 보호했습니다" 창이 뜰 때
> 1. **추가 정보** 클릭
> 2. **실행** 버튼 클릭
>
> #### 위 방법으로도 안 될 때 (파일 속성 차단해제)
> 1. 다운로드된 `.exe` 파일 **우클릭** → **속성**
> 2. 하단 "보안" 항목에서 **차단 해제** 체크박스 체크
> 3. **확인** → 다시 실행
>
> #### 그래도 안 될 때 — PowerShell 명령어로 강제 차단해제
> Windows 검색에서 **PowerShell**을 찾아 **관리자 권한으로 실행** 후 아래 명령 입력:
> ```powershell
> Unblock-File -Path "C:\Users\사용자이름\Downloads\Kidsnote_Memories_Saver_V1.00.exe"
> ```
> (경로를 실제 다운로드 위치로 변경하세요)
>
> #### 또는 Python으로 직접 실행 (가장 확실한 방법)
> 아래 **"방법 2: Python으로 직접 실행"** 을 따라하세요. SmartScreen 차단이 없습니다.
...
```
├── kidsnote_saver.py            # 메인 GUI 프로그램
├── kidsnote_engine.py           # 키즈노트 스크래핑 자동화 코어 엔진
├── test_scenario_runner.py      # E2E 테스트 공통 시나리오 모듈
├── test_simple_check.py         # 3개월 10개 다운로드 심플 테스트 쉘
├── test_full_check.py           # 전체 기간 무제한 풀스크래핑 테스트 쉘
├── build_release_v1.bat         # V1.00 정식 릴리스 빌드 및 패키징 스크립트
└── README.md                    # 이 파일
```
...
> 현재는 Windows 전용입니다. Python이 설치되어 있다면 `python kidsnote_saver.py`로 직접 실행은 가능할 수 있습니다.
