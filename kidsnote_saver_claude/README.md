# 🧒 Kidsnote Memories Saver (키즈노트 추억 저장기) V1.02
...
1. GitHub의 [Releases](../../releases) 페이지에서 최신 **`Kidsnote_Release_V1.02.zip`** 파일을 다운로드합니다.
2. 압축을 풀면 `Kidsnote_Memories_Saver_V1.02` 폴더가 나옵니다. 폴더 안의 **`Kidsnote_Memories_Saver_V1.02.exe`** 를 더블클릭하여 실행합니다. (폴더 안의 다른 파일들은 프로그램 구동용이니 지우지 마세요.)
3. **Microsoft Edge 브라우저**만 사용자의 윈도우 PC에 설치되어 있으면 끝입니다! (별도의 프로그램이나 드라이버 설치가 절대 필요 없습니다.)

> [!NOTE]
> V1.01부터 폴더(onedir) 방식으로 배포합니다. 단일 exe 방식보다 시작이 빠르고,
> 회사 PC처럼 보안 프로그램이 엄격한 환경에서 종료 시 "임시폴더 삭제 실패" 경고가 뜨지 않습니다.
...
- 🎯 **`Kidsnote_Release_V1.02.zip`** (빌드 스크립트가 자동으로 만들어준 100% 무설치 완성본입니다.)

빌드는 `build_release_v1.bat` 실행 한 번이면 됩니다 (기본: 폴더 배포 방식 onedir).
단일 exe가 꼭 필요하면 `build_release_v1.bat onefile`로 빌드할 수 있지만,
시작 속도가 느리고 일부 보안 환경에서 종료 시 임시폴더 경고가 뜰 수 있어 권장하지 않습니다.
...
> 프로그램 실행 자체는 막히지 않습니다! 가장 흔한 웹 드라이버 에러를 방지하기 위해 배포 폴더 안에 Edge 통신용 엔진을 함께 담아서(Bundling) 빌드했습니다. Edge 브라우저만 창이 뜰 수 있다면 구동됩니다.
> 다만 일부 회사 네트워크(프록시)는 **사진/동영상 원본 파일 다운로드**를 차단할 수 있습니다. 이 경우 프로그램이 시작 전에 미리 경고를 띄워주며, PDF 저장 모드는 정상 동작하니 사진 원본은 가정 네트워크에서 받으시길 권장합니다.
