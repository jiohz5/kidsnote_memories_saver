# 🧒 Kidsnote Memories Saver (키즈노트 추억 저장기) V1.05
...
- 🆕 **PDF+사진+동영상 한 번에 저장 (New!)** — 기존에는 PDF 저장과 사진/동영상 저장 중 하나만 골라야 했지만, 이제 한 번의 다운로드로 둘 다 같은 폴더에 저장할 수 있습니다.
- 🛡️ **키즈노트 웹 오류 자동 복구 (New!)** — 조회 중 키즈노트 사이트 자체 오류 화면("아이쿠! 에러가 발생했습니다" 등)이 뜨면 즉시 감지해 화면을 새로 고치고 최대 3회까지 자동으로 다시 시도합니다.
- 🌐 **미디어 다운로드 우회 경로 강화 (New!)** — 사진/동영상 서버가 브라우저 보안 정책(CORS)이나 사내망 프록시로 막혀 있어도 CDP(디버깅 프로토콜) 경유 방식으로 원본을 받아옵니다.
- 🚀 **완벽한 오프라인 구동** — 사내망/방화벽 환경에서도 차단 없이 무조건 실행되도록 Edge WebDriver를 프로그램 내부에 완전히 내장했습니다.
- 🖥️ **4K 고해상도 완벽 지원** — High DPI 자동 스케일링을 지원하여 4K 모니터에서도 UI와 글씨가 작아지지 않고 선명하게 보입니다.
...
1. GitHub의 [Releases](../../releases) 페이지에서 최신 **`Kidsnote_Release_V1.05.zip`** 파일을 다운로드합니다.
2. 압축을 풀면 `Kidsnote_Memories_Saver_V1.05` 폴더가 나옵니다. 폴더 안의 **`Kidsnote_Memories_Saver_V1.05.exe`** 를 더블클릭하여 실행합니다. (폴더 안의 다른 파일들은 프로그램 구동용이니 지우지 마세요.)
...
- 🎯 **`Kidsnote_Release_V1.05.zip`** (빌드 스크립트가 자동으로 만들어준 100% 무설치 완성본입니다.)
...
> 사진/동영상 원본 다운로드가 회사 네트워크(프록시)에 막히는 경우를 대비해, V1.05부터는 **직접 다운로드 → CDP(디버깅 프로토콜) 경유 → 브라우저 경유 → 화면 캡처** 순으로 여러 우회 경로를 자동으로 시도합니다. 브라우저 화면에 사진이 보이는 환경이라면 대부분 원본을 받아올 수 있습니다.
> 그래도 전부 막히는 극단적인 환경이라면 PDF 저장 모드는 정상 동작하니, 사진 원본은 가정 네트워크에서 받으시길 권장합니다.
